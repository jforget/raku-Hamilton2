...
sub fill($at, :$lang
  ...
  my %node-histo;
  for @areas -> $area {
    %node-histo{$area<nb_macro_paths_1>}<nb>++;
    %node-histo{$area<nb_macro_paths_1>}<nodes>.push($area<code>);
  }
  my @colours;
  ...
  for @areas -> $area {
    $area<name> ~= ": $area<nb_macro_paths_1>";
    my $i = @colour-scheme.first( { $_<min> ≤ $area<nb_macro_paths_1> ≤ $_<max> }, :k);
  ...
  for @borders -> $border {
    if $border<code_f> lt $border<code_t> {
      %edge-histo{$border<nb_paths_1>}<nb>++;
      %edge-histo{$border<nb_paths_1>}<edges>.push("$border<code_f> → $border<code_t>");
    }
  }
  ...
  for @borders -> $border {
    my $i = @colour-scheme.first( { $_<min> ≤ $border<nb_paths_1> ≤ $_<max> }, :k);
...
our sub render(Str $lang
            ...
            ,     :@ico-links
            , Str :$query-string) {
  my &filling = anti-template :source("html/macro-stat1.$lang.html".IO.slurp), &fill;
