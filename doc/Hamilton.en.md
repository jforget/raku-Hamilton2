...
and an incorrect `JAP → MNG` border.
...
Initialising the Paris Subway Map, `init-ratp.raku`
---------------------------------------------------
...
RER  lines  (regional  network)  for their  parts  within  Paris.  For
example, line B  extends from "Gare du Nord"  to "Cité Universitaire",
discarding  "La Plaine  Stade de  France"  and northward,  as well  as
"Gentilly" and southward. The only exception is "La Défense", which is
outside Paris, yet  has a connection with subway line  1. I also added
walking connections  through corridors. On  the other hand, I  did not
include tram lines  and train lines. For reasons  explained later, RER
line D does not appear in the map.
...
copied to `Areas` records if some  conditions apply. When a station is
...
Platonic solids, the initialisation programs  need a text file to load
these city names.  On the other hand, for elementary  graphs, the node
name is automatically the  same as the node code, so  there is no need
for a text file.

For  the dodecahedron,  the  coordinates from  file `icosian.txt`  are
polar coordinates, not cartesian coordinates.  The same applies to the
elementary graphs. On the other  hand, the tetrahedron, the octahedron
and the hexahedron (or cube)  are drawn using cavalier perspective, so
the coordinates  from file  `platonic.txt` are  cartesian coordinates.
The  numeric  values  have  nothing  to do  with  the  longitudes  and
latitudes of the cities mentioned in these graphs.

When you  store floating values  without a fractional part  in SQLite,
when  you read  these values  back, SQLite  gives you  integer values,
incompatible  with Raku's  `Num` type.  Therefore, the  initialisation
programs add a small fractional part  so that when you read the values
afterwards,  SQLite will  give  you floating  numbers compatible  with
