...
Programme d'initialisation de la carte de la RATP, `init-ratp.raku`
-------------------------------------------------------------------
...
Graphes abstraits, `init-ico.raku`, `init-platon.raku` et `init-elem.raku`
--------------------------------------------------------------------------

Comme Édouard Lucas  a attribué des noms aux sommets  du dodécaèdre du
jeu icosien et comme j'ai repris  la même idée pour les autres solides
platoniciens,  les programmes  correspondants  utilisent des  fichiers
séquentiels  pour charger  ces  noms. En  revanche,  pour les  graphes
élémentaires,  le nom  est par  construction  égal au  code, donc  pas
besoin de fichier texte.

Pour le dodécaèdre, les coordonnées provenant du fichier `icosian.txt`
sont des  coordonnées polaires,  pas des coordonnées  cartésiennes. Le
même principe est  repris pour les graphes  élémentaires. En revanche,
le  tétraèdre, l'octaèdre  et  l'hexaèdre (cube)  sont représentés  en
perspective cavalière,  donc les  coordonnées fournies par  le fichier
`platonic.txt`   sont  des   coordonnées  cartésiennes.   Les  valeurs
numériques n'ont  rien à  voir avec  la longitude  et la  latitude des
villes nommées dans ces graphes.

Si l'on stocke dans une base  SQLite un nombre flottant dont la partie
fractionnaire est nulle, lors des lectures ultérieures SQLite fournira
des valeurs entières,  donc incompatibles avec le type  `Num` de Raku.
Donc les programmes d'initialisation ajoutent une partie fractionnaire
artificielle pour  que SQLite fournisse effectivement  du `Num`. Cette
